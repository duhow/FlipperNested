...
uint8_t nested_crypto1_bit(NestedCrypto1* crypto1, uint8_t in, int is_encrypted) {
    furi_assert(crypto1);
    uint8_t out = nested_crypto1_filter(crypto1->odd);
...
uint8_t nested_crypto1_byte(NestedCrypto1* crypto1, uint8_t in, int is_encrypted) {
    ...
    for(uint8_t i = 0; i < 8; i++) {
        out |= nested_crypto1_bit(crypto1, FURI_BIT(in, i), is_encrypted) << i;
    }
    return out;
}
...
uint32_t nested_crypto1_word(NestedCrypto1* crypto1, uint32_t in, int is_encrypted) {
    ...
    for(uint8_t i = 0; i < 32; i++) {
        out |= (uint32_t)nested_crypto1_bit(crypto1, BEBIT(in, i), is_encrypted) << (24 ^ i);
    }
    return out;
}
...
void nested_crypto1_decrypt(
    ...
    if(encrypted_data_bits < 8) {
        ...
        decrypted_byte |= (nested_crypto1_bit(crypto, 0, 0) ^ FURI_BIT(encrypted_data[0], 0)) << 0;
        decrypted_byte |= (nested_crypto1_bit(crypto, 0, 0) ^ FURI_BIT(encrypted_data[0], 1)) << 1;
        decrypted_byte |= (nested_crypto1_bit(crypto, 0, 0) ^ FURI_BIT(encrypted_data[0], 2)) << 2;
        decrypted_byte |= (nested_crypto1_bit(crypto, 0, 0) ^ FURI_BIT(encrypted_data[0], 3)) << 3;
        decrypted_data[0] = decrypted_byte;
    } else {
        for(size_t i = 0; i < encrypted_data_bits / 8; i++) {
            decrypted_data[i] = nested_crypto1_byte(crypto, 0, 0) ^ encrypted_data[i];
        }
    }
}
...
void nested_crypto1_encrypt(
    ...
    if(plain_data_bits < 8) {
        ...
        for(size_t i = 0; i < plain_data_bits; i++) {
            encrypted_data[0] |= (nested_crypto1_bit(crypto, 0, 0) ^ FURI_BIT(plain_data[0], i)) << i;
        }
    } else {
        memset(encrypted_parity, 0, plain_data_bits / 8 + 1);
        for(uint8_t i = 0; i < plain_data_bits / 8; i++) {
            encrypted_data[i] = nested_crypto1_byte(crypto, keystream ? keystream[i] : 0, 0) ^
                                plain_data[i];
            encrypted_parity[i / 8] |=
                (((nested_crypto1_filter(crypto->odd) ^ oddparity8(plain_data[i])) & 0x01)
                 << (7 - (i & 0x0007)));
        }
    }
}